...
class PipelineController:
    ...
    def handle_post_pipeline(self, *, repo_path: str, choice: str) -> dict:
        ...
        if choice == "4":
            if not self.support.get_waiting_next_task_approval(repo_path=repo_path):
    ...
    def handle_next_task_approval(self, *, repo_path: str, choice: str) -> dict:
        ...
        if choice == "2":
            session_name = self.support.get_session_name(repo_path=repo_path)
            ...
            store.update_fields(
                ...
                previous_menu=MENU_NEXT_TASK_APPROVAL,
                last_message=str(result.get("message", "")).strip() or "今回は次 task を作成しません。",
            )
