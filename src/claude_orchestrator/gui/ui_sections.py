...
def connect_main_window_signals(window: Any) -> None:
    ...
    window.btn_request_stop_after_current_task.clicked.connect(
        window.on_request_stop_after_current_task
    )
    ...
    window.btn_run_plan_director.clicked.connect(window.on_run_plan_director)
    ...
    window.btn_approve_next_task.clicked.connect(
    ...
    window.pipeline_auto_approval_radio.toggled.connect(
        window.on_pipeline_approval_mode_changed
    )
    window.pipeline_auto_approval_warning_toggle_btn.clicked.connect(
        window.on_toggle_pipeline_approval_warning
    )
    window.pipeline_task_summary_toggle_btn.clicked.connect(
        window.on_toggle_pipeline_task_summary_detail
    )
    window.pipeline_development_mode_combo.currentIndexChanged.connect(
        window.on_development_mode_changed
    )
