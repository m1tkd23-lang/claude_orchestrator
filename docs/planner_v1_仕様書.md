...
### 8.4.1 各 proposal の任意情報

以下のフィールドは任意（optional）であり、schema の required に含まれない。省略可能である。

- `why_now`: この task を今やるべき理由（string）。なぜこのタイミングで着手するかを 1〜2 文で記述する。
- `depends_on`: この task の前提となる task_id のリスト（array of string）。省略するか、空配列 `[]` を指定する。例: `["TASK-0001", "TASK-0002"]`

いずれも planner が判断しやすい場合のみ記述し、不明な場合は省略してよい。
...
### 9.1 任意フィールド（why_now / depends_on）を含む JSON イメージ

why_now および depends_on を付与する場合の例を示す。いずれも任意項目のため、省略しても schema エラーにならない。
  ...
  "proposals": [
    {
      "proposal_id": "PLAN-0001",
      "title": "Claude実行モニタの文言重複を整理する",
      "description": "advanced to next role の重複表示を削減し、文言を整理して視認性を改善する。",
      "priority": "high",
      "reason": "既に主線機能は成立しているため、短時間で UX を改善できる。",
      "context_files": [
        "src\\claude_orchestrator\\gui\\auto_run_worker.py",
      ...
      "why_now": "主線実装が完了した直後であり、今着手することで手戻りが最小になる。",
