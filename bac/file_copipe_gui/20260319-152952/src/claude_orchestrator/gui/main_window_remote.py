...
class MainWindowRemoteMixin:
    ...
    def on_start_remote_operator(self) -> None:
        try:
            repo_path = require_repo_path(self)
            handle_repo_changed(self, repo_path)
            ...
            append_log(self, "[INFO] sending remote operator prompt...")
            ...
            append_log(self, "[INFO] remote operator prompt sent.")

            show_info(
                self,
                "Remote Operator",
                "Remote Operator プロンプトを送信しました。",
            )

        except Exception as exc:
            show_error(self, "Remote Operator 起動エラー", exc)
