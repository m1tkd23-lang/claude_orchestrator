...
class MainWindowRemoteMixin:
    ...
    def _sync_gui_from_repo_state(self, *, auto: bool) -> None:
        ...
        try:
            ...
            current_task_id = get_display_target_task_id(self)

            if current_task_id:
                self._current_task_id = current_task_id
                load_selected_task_detail(self, current_task_id)
    ...
    def _resolve_remote_target_task_id(self, info: object) -> str:
        ...
        # selected_source_task_id は親 task を表すことがあり、
        # 次 task 実行後に古い task へ表示を戻す原因になるため、
        # 通常の GUI 同期対象には使わない。
        local_selected_task_id = str(getattr(self, "_selected_task_id", "")).strip()
        if local_selected_task_id:
            return local_selected_task_id

        local_current_task_id = str(getattr(self, "_current_task_id", "")).strip()
        if local_current_task_id:
            return local_current_task_id
