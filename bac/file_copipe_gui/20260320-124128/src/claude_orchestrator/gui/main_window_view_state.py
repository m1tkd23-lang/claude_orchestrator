...
class MainWindowViewStateMixin:
    ...
    def _set_auto_run_controls_enabled(self, enabled: bool) -> None:
        ...
        self.btn_generate_next_tasks.setEnabled(
            enabled and not self._planner_active and not self._plan_director_active
        )
    ...
    def _set_planner_controls_enabled(self, enabled: bool) -> None:
        self.btn_generate_next_tasks.setEnabled(
            enabled and not self._auto_run_active and not self._plan_director_active
        )
        self.btn_run_plan_director.setEnabled(
            ...
            and bool(self._planner_report)
        )
        self.planner_role_combo.setEnabled(
            enabled and not self._auto_run_active and not self._planner_active
        )
