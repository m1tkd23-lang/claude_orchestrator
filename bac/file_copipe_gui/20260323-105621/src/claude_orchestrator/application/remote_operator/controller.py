...
class RemoteOperatorController:
    ...
    def _handle_proposal_list(self, *, repo_path: str, choice: str) -> dict:
        ...
        planner_role = self._get_active_planner_role(repo_path=repo_path)
    ...
    def _handle_selected_proposal(self, *, repo_path: str, choice: str) -> dict:
        ...
        proposal_planner_role = self._get_selected_proposal_planner_role(repo_path=repo_path)
    ...
    def _handle_pipeline_settings(self, *, repo_path: str, choice: str) -> dict:
        ...
        if choice == "1":
            store.update_fields(
                ...
                last_message="active_planner_role を planner_safe にしました。",
            )
            return self._render_result(
                repo_path=repo_path,
                menu=MENU_PIPELINE_SETTINGS,
        ...
        if choice == "2":
            store.update_fields(
                ...
                current_menu=MENU_PIPELINE_SETTINGS,
                last_message="active_planner_role を planner_improvement にしました。",
            )
            return self._render_result(
                repo_path=repo_path,
                menu=MENU_PIPELINE_SETTINGS,
        ...
        if choice == "6":
            ...
            return self._render_result(
                repo_path=repo_path,
                menu=MENU_PIPELINE_SETTINGS,
                last_message="development_mode を mainline にしました。",
            )
        ...
        if choice == "7":
            ...
            return self._render_result(
                repo_path=repo_path,
                menu=MENU_PIPELINE_SETTINGS,
                last_message="development_mode を maintenance にしました。",
            )
    ...
    def _run_plan_director_and_render(
        ...
        *,
        ...
        selected_planner_role = str(
            report.get("selected_planner_role", "")
        ).strip() or self._get_active_planner_role(repo_path=repo_path)
    ...
    def _create_task_from_plan_director_and_render(
        ...
        *,
        ...
        store.update_fields(
            ...
            previous_menu=previous_menu,
            waiting_next_task_approval=False,
            active_planner_role=selected_planner_role or self._get_active_planner_role(repo_path=repo_path),
        ...
        *,
        ...
        if current_menu == MENU_POST_PIPELINE:
            ...
            return self.renderer.render_post_pipeline_menu(
                ...
                active_planner_role=self._get_active_planner_role(repo_path=repo_path),
                ...
                development_mode=self._get_development_mode(repo_path=repo_path),
                last_message=last_message,
            )
    ...
    @staticmethod
    def _get_active_planner_role(*, repo_path: str) -> str:
        store = RemoteSessionStore(repo_path=repo_path)
        payload = store.load()
    ...
    @staticmethod
    def _get_selected_proposal_planner_role(*, repo_path: str) -> str:
        store = RemoteSessionStore(repo_path=repo_path)
        payload = store.load()
        planner_role = str(
            payload.get("selected_proposal_planner_role", "")
        ).strip()
        if planner_role in PLANNER_ROLES:
            return planner_role
        return RemoteOperatorController._get_active_planner_role(repo_path=repo_path)

    def _get_development_mode(self, *, repo_path: str) -> str:
    ...
    def _set_development_mode(self, *, repo_path: str, development_mode: str) -> None:
    ...
    def _normalize_development_mode(self, value: str) -> str:
