...
InitProjectUseCase.execute(repo_path, force=False) を実行

既に存在する場合はエラー表示

将来的に force は別導線にするが、MVPでは通常初期化のみ
...
ShowNextUseCase.execute(repo_path, task_id) を実行
...
ValidateReportUseCase.execute(repo_path, task_id, role) を実行
...
AdvanceTaskUseCase.execute(repo_path, task_id) を実行
...
approve で completed 確認

11-2. reviseフロー

director report の final_action = revise

advance

cycle が +1 される

next_role が implementer に戻る
