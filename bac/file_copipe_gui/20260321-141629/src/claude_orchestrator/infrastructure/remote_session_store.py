...
class RemoteSessionStore:
    ...
    VALID_PLANNER_ROLES = {"planner_safe", "planner_improvement"}
    ...
    def load_info(self) -> RemoteSessionInfo:
        ...
        return RemoteSessionInfo(
            ...
            stop_after_current_task_requested=bool(
                payload["stop_after_current_task_requested"]
            ),
            ...
            last_plan_director_selection_reason=str(
                payload["last_plan_director_selection_reason"]
            ),
    ...
    def mark_started(
        ...
        *,
        ...
        current["selected_proposal_planner_role"] = self.DEFAULT_PLANNER_ROLE
    ...
    def _build_default_payload(self) -> dict:
        return {
            ...
            "selected_proposal_planner_role": self.DEFAULT_PLANNER_ROLE,
    ...
    def _normalize_payload(self, payload: dict) -> dict:
        ...
        last_role = str(normalized.get("last_planner_role", "")).strip()
        active_role = str(normalized.get("active_planner_role", "")).strip()
        selected_role = str(
            normalized.get("selected_proposal_planner_role", "")
        ).strip()

        if last_role not in self.VALID_PLANNER_ROLES:
            normalized["last_planner_role"] = self.DEFAULT_PLANNER_ROLE
        if active_role not in self.VALID_PLANNER_ROLES:
            normalized["active_planner_role"] = normalized["last_planner_role"]
        if selected_role not in self.VALID_PLANNER_ROLES:
            normalized["selected_proposal_planner_role"] = normalized[
                "active_planner_role"
            ]
