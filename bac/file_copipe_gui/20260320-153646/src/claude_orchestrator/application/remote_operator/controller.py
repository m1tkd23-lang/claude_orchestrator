...
class RemoteOperatorController:
    ...
    def _handle_post_run(self, *, repo_path: str, choice: str) -> dict:
        ...
        if status == "completed":
            return self._enter_post_pipeline(
                ...
                source_task_id=task_id,
                previous_menu=MENU_POST_RUN,
                last_message=f"{task_id} は completed のため、後工程メニューへ移動しました。",
            )
    ...
    def _handle_post_pipeline(self, *, repo_path: str, choice: str) -> dict:
        ...
        if choice == "2":
            ...
            ).strip() or str(payload.get("last_planner_role", "planner_safe")).strip()

            approval_mode = str(payload.get("approval_mode", "manual")).strip() or "manual"
            ...
            store.update_fields(
                ...
                waiting_next_task_approval=(
                    decision == "adopt" and approval_mode == "manual"
                ),
    ...
    def _create_task_from_plan_director_and_render(
        ...
        *,
        ...
        store.update_fields(
            ...
            previous_menu=previous_menu,
            waiting_next_task_approval=False,
            last_message=f"{source_task_id} から {created_task_id} を作成しました。",
        )
        return self._render_result(
            repo_path=repo_path,
            menu=MENU_CREATED_TASK,
