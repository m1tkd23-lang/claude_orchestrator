...
class MainWindow(QMainWindow):
    ...
    def on_run_claude_step(self) -> None:
        try:
            ...
            runtime = TaskRuntime(target_repo=Path(repo_path).resolve(), task_id=task_id)
            initial_state = runtime.load_state_json()
            if str(initial_state.get("next_role")) == "none":
                self._set_execution_state("completed")
                self._set_execution_step("idle")
                self._set_execution_role("none")
                self._set_execution_cycle(str(initial_state.get("cycle", "")))
                self._append_execution_summary("この task はすでに completed です。")
                append_log(self, f"[INFO] task already completed: {task_id}")
                return

            self._set_execution_state("running")
            self._set_execution_step("starting")
            self._set_execution_role(str(initial_state.get("next_role", "")))
            self._set_execution_cycle(str(initial_state.get("cycle", "")))
            self.execution_summary_edit.clear()
            self.validation_result_edit.clear()

            append_log(self, f"[INFO] auto run started: task_id={task_id}")

            loop_count = 0
            while True:
                loop_count += 1
                step_result = self._run_single_auto_step(repo_path=repo_path, task_id=task_id)

                new_status = str(step_result["status"])
                new_next_role = str(step_result["next_role"])
                new_cycle = int(step_result["cycle"])

                if new_next_role == "none" or new_status == "completed":
                    self._set_execution_state("completed")
                    self._set_execution_step("stopped")
                    self._set_execution_role("none")
                    self._set_execution_cycle(str(new_cycle))
                    self._append_execution_summary(
                        f"loop={loop_count}: task completed"
                    )
                    append_log(
                        self,
                        f"[INFO] auto run completed: task_id={task_id}, cycle={new_cycle}",
                    )
                    break

                self._set_execution_state("running")
                self._set_execution_step("next role ready")
                self._set_execution_role(new_next_role)
                self._set_execution_cycle(str(new_cycle))
                self._append_execution_summary(
                    f"loop={loop_count}: advanced to role={new_next_role}, cycle={new_cycle}"
                )
    ...
    def _run_single_auto_step(self, *, repo_path: str, task_id: str) -> dict:
        ...
        self._append_execution_summary(
            f"role={role}, cycle={cycle}: show-next completed"
        )
        append_log(
            self,
            "[INFO] show-next completed: "
        ...
        self._set_execution_step("claude")
        append_log(
            self,
        ...
        self._append_execution_summary(
            f"role={role}, cycle={cycle}: report saved"
        )
        ...
        self._set_validation_result_text(validate_result)
        append_log(
            self,
            "[INFO] validate-report success: "
            f"task_id={task_id}, role={validate_result['role']}, cycle={validate_result['cycle']}",
        )
        self._append_execution_summary(
            f"role={role}, cycle={cycle}: validate success"
        )

        self._set_execution_step("advance")
        advance_result = AdvanceTaskUseCase().execute(repo_path=repo_path, task_id=task_id)
        append_log(
            self,
            "[INFO] advance completed: "
            f"task_id={task_id}, "
        ...
        self._append_execution_summary(
            f"role={role}, cycle={cycle}: advance success -> "
            f"next_role={advance_result['next_role']}, status={advance_result['status']}"
        )
    ...
    def _set_validation_result_text(self, result: dict) -> None:
        text = (
            f"valid: {result['valid']}\n"
            f"role: {result['role']}\n"
            f"cycle: {result['cycle']}\n"
    ...
    def _append_execution_summary(self, message: str) -> None:
        self.execution_summary_edit.appendPlainText(message)