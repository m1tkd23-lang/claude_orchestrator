...
def build_pipeline_task_summary(window: Any) -> dict[str, str]:
    return {
        "task_id": _get_text_attr(window, "detail_task_id", "current_task_id_edit"),
        ...
        "cycle": _get_text_attr(window, "detail_cycle", "current_task_cycle_edit", "execution_cycle_edit"),
        ...
        "stop_reservation": "あり" if bool(getattr(window, "_stop_after_current_task_requested", False)) else "なし",
    }
...
def build_pipeline_report_summaries(window: Any) -> dict[str, str]:
    ...
    result: dict[str, str] = {}
    for role in _STANDARD_ROLES:
        result[role] = summarize_latest_standard_report(latest_reports.get(role))

    result["planner"] = summarize_planner_report(
        getattr(window, "_planner_report", None),
        getattr(window, "_planner_state_store", None),
        _get_current_planner_role(window),
    )
    result["plan_director"] = summarize_plan_director_report(
        getattr(window, "_plan_director_report", None)
    )
    return result
...
def summarize_plan_director_report(plan_director_report: dict | None) -> str:
    ...
    if lines:
        lines.append("")
    lines.append(selection_reason if selection_reason else "(selection_reason なし)")
    return "\n".join(lines).strip()
...
def _build_standard_role_state(
    ...
    next_role = _get_text_attr(window, "detail_next_role", "current_task_next_role_edit")
    cycle_text = _get_text_attr(window, "execution_cycle_edit", "detail_cycle", "current_task_cycle_edit")
