...
class CreateTaskFromProposalUseCase:
    ...
    def _proposal_to_task_fields(proposal: dict) -> dict:
        ...
        depends_on = [str(x).strip() for x in (proposal.get("depends_on", []) or []) if str(x).strip()]
        context_files = [str(x).strip() for x in (proposal.get("context_files", []) or []) if str(x).strip()]
        constraints = [str(x).strip() for x in (proposal.get("constraints", []) or []) if str(x).strip()]
