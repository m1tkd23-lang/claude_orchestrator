...
class MainWindowRemoteMixin:
    ...
    def _load_remote_session_info_safe(self) -> None:
        ...
        try:
            ...
            # session_name 自動補完（未入力時のみ）
            if not self.remote_session_name_edit.text().strip():
                self.remote_session_name_edit.setText(
                    self._generate_default_session_name(repo_path)
                )
